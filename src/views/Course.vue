        ...
        <div class="" v-for="section in sections" :key="section.fields.Title">
          ...
          <div :id="section.id" style="display:none">
            <div v-if="track || !user">
              <div
                class=""
                v-for="subject in filteredSubjects(section)"
                :key="subject.fields.Title"
                v-on:click="loadFullSubject(subject)"
              >
                <input
                  type="radio"
                  :id="subject.id"
    ...
    .then((response) => {
      ...
      switch (settings.bind) {
        ...
        case "studentTrack":
          ...
          if (!viewStatus.track.fields.Subjects)
            viewStatus.track.fields.Subjects = [];
          //how much finished sofar?
          for (let x = 0; x < viewStatus.subjects.length; x++) {
            let tsub = viewStatus.track.fields.Subjects.filter(function(sub) {
              return sub == viewStatus.subjects[x].id;
            });
            if (tsub.length > 0) {
              var tsec = viewStatus.sections.filter(function(sec) {
                return sec.id == viewStatus.subjects[x].fields.Section;
              });
              if (tsec.length > 0) {
...
export default {
  ...
  methods: {
    ...
    getStudentTrack: function(compareSubject) {
      ...
      if (
        this.track.fields.Subjects.filter(function(subject) {
          if (subject == compareSubject.id) return true;
          else return false;
        }).length > 0
      )
        return true;
    ...
    toggleSection: function(section) {
      ...
      if (document.getElementById(section.id).style.display == "none") {
        ...
        document.getElementById("arrow" + section.id).className =
          "fa fa-angle-double-up";
        this.selectedSubject = null;

        if (this.selectedSection) {
          document.getElementById(this.selectedSection.id).style.display =
            "none";
          document.getElementById("arrow" + this.selectedSection.id).className =
